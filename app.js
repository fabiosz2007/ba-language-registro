...
async function sincronizarCursosEnAgenda(profesoraId) {
    ...
    // Eliminar TODOS los bloques "asignados" existentes
    ...
    if (deleteError) {
        console.error('Error eliminando bloques asignados:', deleteError);
    } else {
        console.log('✓ Bloques asignados eliminados');
    }
    ...
    // Verificar si tiene bloques disponibles, si no, crearlos
    const { data: disponibles, count } = await supabase
        .from('disponibilidad_profesoras')
        .select('id', { count: 'exact', head: true })
        .eq('profesora_id', profesoraId)
        .eq('tipo', 'disponible');
    
    console.log('Bloques disponibles existentes:', count);
    
    if (count === 0) {
        console.log('Creando bloques disponibles...');
        ...
        if (bloquesDisponibles.length > 0) {
            ...
            console.log('✓ Bloques disponibles creados');
        }
    }
...
function generarVistaAgenda(bloques) {
    ...
    bloques.forEach(bloque => {
        if (bloquePorDia[bloque.dia_semana]) {
