// CONFIGURACION - Sistema BA Language
const SUPABASE_URL = 'https://nqidcovrjzywiilzdffy.supabase.co';
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im5xaWRjb3Zyanp5d2lpbHpkZmZ5Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzExNjczMjEsImV4cCI6MjA4Njc0MzMyMX0const supabase = window.supabase.createClient(SUPABASE_URL, SUPABASE_KEY);
...
showError('loginError', ' Te enviamos un email con instrucciones para recuperar tu }
}
...
option.textContent = `${curso.codigo_curso} - ${curso.idioma} ${curso.nivel} - ${option.dataset.duracion = curso.duracion_horas;
...
div.style.cssText = 'padding: 1rem; margin: 0.5rem 0; background: white; border-radius: div.innerHTML = `
<div style="font-weight: 600; margin-bottom: 0.75rem;">${participante.nombre}</<div style="display: flex; flex-direction: column; gap: 0.5rem;">
<label style="display: flex; align-items: center; cursor: pointer; padding: <input type="radio" name="asist_${participante.id}" value="presente" <span>Presente</span>
</label>
<label style="display: flex; align-items: center; cursor: pointer; padding: <input type="radio" name="asist_${participante.id}" value="ausente_con_<span>Ausente con aviso</span>
</label>
<label style="display: flex; align-items: center; cursor: pointer; padding: <input type="radio" name="asist_${participante.id}" value="ausente_sin_<span>Ausente sin aviso</span>
</label>
</div>
`;
...
if (estado === 'dictada') {
const participantesContainers = document.querySelectorAll('#participantesList .participante-const asistencias = [];
...
const todosAusentesConAviso = asistencias.every(a => a.estado_asistencia === 'ausente_if (todosAusentesConAviso && !esRecup) {
// Alerta importante: todos ausentes con aviso
const confirmar = confirm(
' ATENCIÓN: Todos los participantes están marcados como "Ausentes con aviso".\'Recordá que según el contrato con el cliente, estos participantes tienen 'Es importante que coordines la recuperación de esta clase con los participantes.\'¿Querés continuar guardando esta clase de todas formas?'
);
if (!confirmar) {
...
.insert(asistencias);
if (asistError) {
showAlert('asistenciaAlert', 'Error al guardar asistencias: ' + asistError.message, return;
}
}
}
...
<td><span class="badge ${cliente.activo ? 'badge-success' : 'badge-danger'}">${<td class="action-buttons">
<button class="btn btn-warning btn-small" onclick="editCliente('${cliente.<button class="btn btn-danger btn-small" onclick="deleteCliente('${cliente.</td>
`;
tbody.appendChild(tr);
});
} else {
tbody.innerHTML = '<tr><td colspan="4" class="empty-state"><div class="empty-state-icon"> }
}
...
if (!nombre || !codigo) {
showModalAlert('clienteModalAlert', 'Por favor completá todos los campos obligatorios', return;
}
if (codigo.length !== 2) {
...
showModalAlert('clienteModalAlert', 'Error al guardar: ' + result.error.message, }
return;
}
...
<button class="btn btn-warning btn-small" onclick="editCurso('${curso.id}')"><button class="btn btn-danger btn-small" onclick="deleteCurso('${curso.id}', </td>
`;
tbody.appendChild(tr);
});
} else {
tbody.innerHTML = '<tr><td colspan="8" class="empty-state"><div class="empty-state-icon"> }
}
...
container.innerHTML = '<p style="color: #999; padding: 1rem;">Primero seleccioná un cliente</return;
}
const { data, error } = await supabase
.from('participantes')
.select('*')
...
div.innerHTML = `
<input type="checkbox" id="cursopart_${participante.id}" value="${participante.<label for="cursopart_${participante.id}">${participante.nombre}</label>
`;
container.appendChild(div);
});
} else {
container.innerHTML = '<p style="color: #999; padding: 1rem;">No hay participantes asignados }
}
...
<input type="time" class="horario-inicio" data-index="${index}" value="${inicio}" </div>
<div class="form-group">
<label>Hora fin</label>
<input type="time" class="horario-fin" data-index="${index}" value="${fin}" placeholder="</div>
<div class="form-group">
<button type="button" class="btn btn-danger btn-small" onclick="eliminarHorario(${</div>
`;
container.appendChild(div);
if (horarioId) {
...
const horaFinStr = `${String(horaFin).padStart(2, '0')}:${String(minutoFin).padStart(2, 'document.querySelector(`.horario-fin[data-index="${index}"]`).value = horaFinStr;
}
...
async function verificarSolapamientoHorarios(profesoraId, nuevosHorarios, cursoIdActual = null) // Obtener todos los cursos de esta profesora (excepto el que estamos editando)
...
showModalAlert('cursoModalAlert', 'Por favor completá todos los campos obligatorios', return;
}
...
showModalAlert('cursoModalAlert', ` Conflicto de horarios: Este curso tiene horarios return;
}
}
...
if (solapamiento) {
showModalAlert('cursoModalAlert', ` Conflicto de horarios: Esta profesora ya tiene return;
}
}
const cursoData = {
...
if (horarioError) {
showModalAlert('cursoModalAlert', 'Error al guardar horarios: ' + horarioError.message, return;
}
}
// Asignar profesora
if (profesoraId) {
...
const checkboxes = document.querySelectorAll('#cursoParticipantes input[type="checkbox"]:// Eliminar asignaciones anteriores
await supabase
.from('cursos_participantes')
.delete()
.eq('curso_id', cursoId);
...
<td><span class="badge ${profesora.es_admin ? 'badge-warning' : 'badge-info'}">${<td><span class="badge ${profesora.activo ? 'badge-success' : 'badge-danger'}">${<td class="action-buttons">
<button class="btn btn-warning btn-small" onclick="editProfesora('${profesora.<button class="btn btn-danger btn-small" onclick="deleteProfesora('${profesora.</td>
`;
tbody.appendChild(tr);
});
} else {
tbody.innerHTML = '<tr><td colspan="6" class="empty-state"><div class="empty-state-icon"> }
}
...
if (!nombre || !email) {
showModalAlert('profesoraModalAlert', 'Por favor completá todos los campos obligatorios', return;
}
const profesoraData = {
nombre: nombre,
email: email,
...
showModalAlert('profesoraModalAlert', 'Error al guardar: ' + result.error.message, }
return;
}
...
<td><span class="badge ${participante.activo ? 'badge-success' : 'badge-danger'}">${<td class="action-buttons">
<button class="btn btn-warning btn-small" onclick="editParticipante('${participante.<button class="btn btn-danger btn-small" onclick="deleteParticipante('${participante.</td>
`;
tbody.appendChild(tr);
});
} else {
...
if (!nombre) {
showModalAlert('participanteModalAlert', 'Por favor ingresá el nombre del participante', return;
}
...
if (duplicadoNombre) {
showModalAlert('participanteModalAlert', ` Ya existe un participante con el nombre return;
}
...
if (duplicadoEmail) {
showModalAlert('participanteModalAlert', ` Ya existe un participante con el return;
}
}
}
...
if (result.error) {
showModalAlert('participanteModalAlert', 'Error al guardar: ' + result.error.message, return;
}
...
document.getElementById('reporteClienteSelect').innerHTML = '<option value="">No return;
}
}
...
<div style="display: flex; justify-content: space-between; align-items: center; margin-<h3 style="color: var(--primary);">Reporte: ${cliente.nombre} - ${getMesNombre(<button class="btn btn-success" onclick="exportarReporteClienteExcel('${clienteId}', </div>
`;
...
html += `
<h4 style="color: var(--secondary); margin-top: 1.5rem;">${curso.codigo_curso} <div class="table-container">
<table>
<thead>
<tr>
<th>Fecha</th>
...
const presentes = asistencias.filter(a => a.estado_asistencia === 'presente').const ausentesConAviso = asistencias.filter(a => a.estado_asistencia const ausentesSinAviso = asistencias.filter(a => a.estado_asistencia const total = asistencias.length;
asistenciaHtml = `${presentes}/${total} presentes`;
if (ausentesConAviso > 0 || ausentesSinAviso > 0) {
asistenciaHtml += `<br><small>${ausentesConAviso} aus. c/aviso, ${}
}
}
html += `
<tr>
...
html += `
<h5 style="margin-top: 1rem; color: var(--dark);">Detalle por Participante:</<div class="table-container">
<table>
<thead>
<tr>
<th>Participante</th>
...
const clasesPresentes = asist ? asist.filter(a => a.estado_asistencia === const ausentesConAviso = asist ? asist.filter(a => a.estado_asistencia === const ausentesSinAviso = asist ? asist.filter(a => a.estado_asistencia === const porcentaje = totalClases > 0 ? ((clasesPresentes / totalClases) * 100).html += `
<tr>
<td>${part.participantes.nombre}</td>
<td>${clasesPresentes} presentes<br><small>${ausentesConAviso} aus. <td>${totalClases}</td>
<td><span class="badge ${porcentaje >= 80 ? 'badge-success' : porcentaje </tr>
`;
}
html += `
</tbody>
...
<small style="color: var(--secondary);">Verificá que hayas registrado asistencias </div>
`;
} else {
...
infoContainer.innerHTML = `<p style="color: var(--dark); font-weight: 600;">Reporte }
}
}
...
const totalHoras = clases.reduce((sum, clase) => sum + parseFloat(clase.duracion_horas), // Calcular desglose de horas
let horasDictadasAPagar = 0; // Clases efectivamente dictadas a participantes presentes let horasSuspendidasSinRecuperar = 0; // Clases con todos ausentes con aviso que NO son let horasCanceladasEmpresa = 0;
...
let html = `
<div class="card" style="margin-top: 2rem;">
<div style="display: flex; justify-content: space-between; align-items: center; margin-<h3 style="color: var(--primary);">Reporte: ${profesora.nombre} - ${getMesNombre(<button class="btn btn-success" onclick="exportarReporteProfesoraExcel('${profesoraId}', </div>
...
<small style="opacity: 0.9;">${horasSuspendidasSinRecuperar.toFixed(1)}h </div>
</div>
<div style="margin: 1rem 0; padding: 1rem; background: #f8f9fa; border-radius: 5px;">
<strong>Desglose completo:</strong><br>
• Clases dictadas (con participantes): ${horasDictadasAPagar.toFixed(1)}h (${• Clases suspendidas (todos ausentes): ${horasSuspendidasSinRecuperar.toFixed(• Canceladas por empresa: ${horasCanceladasEmpresa.toFixed(1)}h (${clasesCanceladasEmpresa} • Canceladas por profesora: ${horasCanceladasProfesora.toFixed(1)}h (${clasesCanceladasProfesora} • Feriados: ${horasFeriado.toFixed(1)}h (${clasesFeriado} clases) - <em>No cobrables</<strong>TOTAL A COBRAR: ${horasAPagar.toFixed(1)}h</strong>
</div>
...
if (esSuspendida) {
estadoBadge += ' <span class="badge" style="background: #FFA500;"> Suspendida</}
html += `
<tr>
<td>${formatFecha(clase.fecha)}</td>
...
<td>${estadoBadge}</td>
<td>${clase.es_recuperacion ? '<span class="badge badge-info">Sí</span>' : '-'}</</tr>
`;
});
html += `
</tbody>
...
worksheet.getCell('A1').font = { size: 14, bold: true, color: { argb: 'FF2C5F7C' worksheet.getCell('A1').alignment = { vertical: 'middle', horizontal: 'center' };
worksheet.mergeCells('B1:G1');
const titleCell = worksheet.getCell('B1');
titleCell.value = 'REGISTRO DE ASISTENCIA';
titleCell.font = { size: 16, bold: true, color: { argb: 'FFFFFFFF' } };
titleCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF2C5F7C' titleCell.alignment = { vertical: 'middle', horizontal: 'center' };
...
cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF4472C4' cell.alignment = { vertical: 'middle', horizontal: 'center' };
cell.border = {
top: { style: 'thin' },
left: { style: 'thin' },
bottom: { style: 'thin' },
...
cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: fillColor cell.alignment = { vertical: 'top', horizontal: 'left', wrapText: true };
cell.border = {
...
.eq('activo', true);
if (participantes && participantes.length > 0) {
const worksheetPart = workbook.addWorksheet(`${curso.codigo_curso}-Part`.substring(// Configurar anchos de columna PRIMERO
...
worksheetPart.getCell('A1').font = { size: 14, bold: true, color: { argb: 'FF2C5F7C' worksheetPart.getCell('A1').alignment = { vertical: 'middle', horizontal: 'center' worksheetPart.mergeCells('B1:E1');
...
titleCellPart.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF2C5F7C' titleCellPart.alignment = { vertical: 'middle', horizontal: 'center' };
...
const headersPart = ['Participante', 'Presentes', 'Aus. c/aviso', 'Aus. s/aviso', headersPart.forEach((header, index) => {
...
.in('clases.curso_id', [curso.id]);
const totalClases = asist ? asist.length : 0;
const clasesPresentes = asist ? asist.filter(a => a.estado_asistencia === const ausentesConAviso = asist ? asist.filter(a => a.estado_asistencia === const ausentesSinAviso = asist ? asist.filter(a => a.estado_asistencia === const porcentaje = totalClases > 0 ? ((clasesPresentes / totalClases) * 100).const rowPart = worksheetPart.getRow(rowIndexPart);
rowPart.values = [
...
cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: fillColorPart cell.alignment = { vertical: 'middle', horizontal: col === 1 ? 'left' cell.border = {
top: { style: 'thin', color: { argb: 'FFD3D3D3' } },
left: { style: 'thin', color: { argb: 'FFD3D3D3' } },
bottom: { style: 'thin', color: { argb: 'FFD3D3D3' } },
right: { style: 'thin', color: { argb: 'FFD3D3D3' } }
...
const blob = new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.const url = window.URL.createObjectURL(blob);
...
['Clases suspendidas:', horasSuspendidasSinRecuperar.toFixed(1), '(Pendiente recuperar)', ['Canc. por empresa:', horasCanceladasEmpresa.toFixed(1), '(No cobrables)', '', '', ''],
['Canc. por profesora:', horasCanceladasProfesora.toFixed(1), '(No cobrables)', '', '', ['Feriados:', horasFeriado.toFixed(1), '(No cobrables)', '', '', ''],
...
<h3 style="color: var(--primary);">Resumen de Facturación - ${getMesNombre(mesNum)} <button class="btn btn-success" onclick="exportarFacturacionExcel('${mes}')"> </div>
<div class="table-container">
<table>
<thead>
<tr>
...
<td colspan="3" style="text-align: right;">TOTALES POR CLIENTE:</<td colspan="2"></td>
</tr>
`;
...
html += `
<tr style="background: var(--primary); color: white; font-weight: bold; <td colspan="3" style="text-align: right;">TOTAL GENERAL:</td>
...
const worksheet = workbook.addWorksheet(cliente.codigo || cliente.nombre.substring(0, // Configurar anchos de columna
...
const headers = ['Concepto / Curso', 'Horas', 'Valor Hora', 'Factor %', 'Neto', 'IVA headers.forEach((header, index) => {
const cell = headerRow.getCell(index + 1);
cell.value = header;
cell.font = { bold: true, color: { argb: 'FFFFFFFF' } };
...
const concepto = `Clases de ${curso.idioma}_${curso.codigo}_${getMesNombre(mesNum)} const row = worksheet.getRow(rowIndex);
...
for (let col = 1; col <= 7; col++) {
const cell = row.getCell(col);
cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: fillColor } cell.alignment = { vertical: 'middle', horizontal: col === 1 ? 'left' : 'right' cell.border = {
top: { style: 'thin', color: { argb: 'FFD3D3D3' } },
left: { style: 'thin', color: { argb: 'FFD3D3D3' } },
bottom: { style: 'thin', color: { argb: 'FFD3D3D3' } },
right: { style: 'thin', color: { argb: 'FFD3D3D3' } }
...
totalRow.getCell(7).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF27AE60' // Bordes en fila de totales
for (let col = 1; col <= 7; col++) {
const cell = totalRow.getCell(col);
cell.fill = cell.fill || { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF8F9FA' cell.border = {
top: { style: 'double' },
bottom: { style: 'double' }
};
...
notaRow.getCell(1).value = 'NOTA: El Factor % permite ajustar el precio (ej: 1.00 = sin notaRow.getCell(1).font = { italic: true, size: 9, color: { argb: 'FF666666' } };
...
const HORAS_AGENDA = ['08:00', '09:00', '10:00', '11:00', '12:00', '13:00', '14:00', '15:00', async function showAgenda() {
...
<thead>
<tr>
<th style="background: var(--primary); color: white; padding: 1rem; text-<th style="background: var(--primary); color: white; padding: 1rem; text-<th style="background: var(--primary); color: white; padding: 1rem; text-<th style="background: var(--primary); color: white; padding: 1rem; text-<th style="background: var(--primary); color: white; padding: 1rem; text-</tr>
</thead>
<tbody>
...
html += '<div style="color: #999; text-align: center; padding: 2rem;">Sin horarios } else {
bloquesDelDia.forEach(bloque => {
const esEditable = bloque.tipo === 'bloqueado';
const deleteBtn = esEditable ?
`<button class="bloque-delete" onclick="eliminarBloque('${bloque.id}')" title="let titulo = '';
let claseInfo = '';
if (bloque.tipo === 'asignado' && bloque.cursos) {
titulo = bloque.cursos.codigo_curso;
claseInfo = bloque.cursos.clientes ? `<div style="font-size: 0.75rem; opacity: } else if (bloque.tipo === 'bloqueado') {
titulo = 'Bloqueado';
} else {
titulo = 'Disponible';
}
html += `
<div class="bloque-horario bloque-${bloque.tipo}" style="position: relative; ${deleteBtn}
<div style="font-weight: 600; font-size: 0.9rem;">${titulo}</div>
<div style="font-size: 0.8rem; opacity: 0.95; margin-top: 0.25rem;">${${claseInfo}
</div>
`;
});
}
...
if (horaDesde >= horaHasta) {
showModalAlert('bloqueoModalAlert', 'La hora de inicio debe ser menor a la hora de fin', return;
}
...
if (solapa) {
showModalAlert('bloqueoModalAlert', 'Este horario se solapa con un bloque existente', return;
}
}
...
const finHueco = ocupado.hora_inicio < horaHasta ? ocupado.hora_inicio : const duracion = calcularDuracionHoras(inicioLibre, finHueco);
if (duracion > 0) {
...
console.log(` ${profesora.nombre}: Solo ${horasTotales.toFixed(1)}h (necesita ${}
}
...
console.error('calcularDuracionHoras: inicio y fin deben ser strings', { inicio, fin return 0;
}
...
<div class="empty-state">
<div class="empty-state-icon"> </div>
<p>No se encontraron profesoras con disponibilidad que cumplan los criterios</</div>
`;
} else {
html = `
<div class="card">
<h3 style="color: var(--primary);">Resultados (${resultados.length} profesora${<p style="color: var(--secondary); margin-bottom: 1rem;">
...
const meses = ['Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', const d = new Date(fecha + 'T00:00:00');
return `${d.getDate()} ${meses[d.getMonth()]} ${d.getFullYear()}`;
}
function getMesNombre(mesNum) {
const meses = ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', return meses[parseInt(mesNum) - 1];
}
